...
public class Productos_vendidos_DAO implements CRUD_productos_vendidos {
    ...
    public List listar() {
        ...
        String sql="select id_producto,nombre_producto,cantidad_producto,precio_unitario_producto,especificaciones,dir_img_producto,tipo_producto.nombre_tipo_producto"
                    +" from productos"
                    +" inner join tipo_producto"
                    +" on productos.id_tipo_producto = tipo_producto.id_tipo_producto ORDER BY nombre_producto";
        try {
            ...
            while(rs.next()){
                ...
                ma.setespecificaciones(rs.getString("especificaciones"));
                ma.setdir_img_producto(rs.getString("dir_img_producto"));
    ...
    public Productos_vendidos list(int id_producto) {
        ...
        try {
            ...
            while(rs.next()){                
                ...
                CJ.setespecificaciones(rs.getString("especificaciones"));
                CJ.setdir_img_producto(rs.getString("dir_img_producto"));
    ...
    public boolean add(Productos_vendidos ma) {
       String sql="insert into productos (nombre_producto,cantidad_producto,precio_unitario_producto,especificaciones,dir_img_producto,id_tipo_producto)values('"+ma.getnombre_producto()+"',"+ma.getcantidad_producto()+","+ma.getprecio_unitario_producto()+",'"+ma.getespecificaciones()+"','"+ma.getdir_img_producto()+"',"+ma.getid_tipo_producto()+")";
        try {
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public boolean edit(Productos_vendidos ma) {
        String sql="update productos set nombre_producto= '"+ma.getnombre_producto()+"',cantidad_producto= "+ma.getcantidad_producto()+",precio_unitario_producto= "+ma.getprecio_unitario_producto()+",especificaciones= '"+ma.getespecificaciones()+"',dir_img_producto= '"+ma.getdir_img_producto()+"',id_tipo_producto= "+ma.getid_tipo_producto()+" where id_producto="+ma.getid_producto();
        try {
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
